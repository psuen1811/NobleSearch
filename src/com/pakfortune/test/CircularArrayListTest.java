...
class CircularArrayListTest {
    ...
    void getYearlyResult() {
        ...
        circularArrayList.shiftLeft(lookup.getIfPresent(SixtyJiaziTable.class, "癸亥").ordinal());

        int temp = lookup.getIfPresent(SixtyJiaziTable.class, "壬戌").ordinal();
        int index = (Integer) circularArrayList.get(temp) % 9;
        String result = lookup.findByValue(Direction.class, index).name();
        Assert.assertEquals("艮", result);
    }
